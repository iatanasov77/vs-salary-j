...
node ( label: 'php-host' ) {
    ...
    def HOST_PREFIX             = 'salaryj-'
    def APP_HOST                = 'salaryj.vankosoft.org'
    ...
    final GIT_URI               = 'github.com/iatanasov77/vs-salary-j.git'
    ...
    def FTP_CREDENTIALS_ID      = 'salaryj-ftp';
    def APP_DIR                 = '/opt/VankosoftProjects/SalaryJ';
    def APP_FTP_USER;
                ...
                sshagent(credentials : ['vps-mini-ssh-root']) {
                    sh """
                        ssh -t -t -l root 164.138.221.242 -o StrictHostKeyChecking=no -p 1022  << ENDSSH
                            ...
                            ${PHP_BIN} -d memory_limit=-1 bin/salary-j cache:clear
